package role

import "github.com/azd1997/ecoin/common"


// 实际使用的Role需在区块链各个版本的客户端软件配置文件中指定。这里只定义基本的格式

// 关于币，基本考虑与人民币等值，需用人民币换购并在系统内流通
// 增值，可以参考银行存款，将增长部分发给积极地用户
// 服务费，暂不设置

// 假设系统启动之初，作价一千万，

// Role 账户角色，作为权限控制
type Role struct {
	NoField             uint8  // 编号，从1开始。role1为创始者，编号不可改，别名可以自定义
	AliasField          string // 名称
	InitialField        common.Coin   // 初始币量
	CoinbaseRewardField common.Coin   // 挖矿奖励交易。B类角色只能是0因为不能挖矿。A类可根据协议设置数值
	GenesisRewardField  common.Coin   // 创始者奖励量
	// ks和es组合可以描述很多种币的增长策略，默认值为ks=不设，es=不设，币量不自增
	EnableKsEsField bool
	KsField []int // 系数值		-x^3+3x^2+x+1 + x^-1   [-1 3 1 1 1]
	EsField []int // 幂指数值 [3 2 1 0 -1]
}

// No 获取角色编号
func (r *Role) No() uint8 {
	return r.NoField
}

// Alias 获取角色的字符名称
func (r *Role) Alias() string {
	return r.AliasField
}

// InitialBalance 初始余额
func (r *Role) InitialBalance() common.Coin {
	return r.InitialField
}

// CoinbaseReward 出块奖励金额
func (r *Role) CoinbaseReward() common.Coin {
	return r.CoinbaseRewardField
}

// GenesisReward 出块奖励金额
func (r *Role) GenesisReward() common.Coin {
	return r.GenesisRewardField
}

// TODO: 转发节点出块有一定收益； 转发节点持续运行自增长（相当于利润）； 普通用户节点持续使用自增长（利润、bonus）


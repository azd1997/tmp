...
func (s *tcpServer) Start() bool {
	...
	if s.ln, err = net.ListenTCP("tcp", lnAddr); err != nil {
		log.Warn("tcp server listen failed:%v", err)
		return false
	}
	...
	for {
		...
		default:
			...
			if conn, err := s.ln.AcceptTCP(); err == nil {
				...
				default:
					log.Warn("tcp server listen accept queue full, drop connection")
					conn.Close()
				}
			}
		}
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				if err == io.EOF {
					log.Info("connection closed by remote:%v\n", c.RemoteAddr())
				} else {
					log.Warn("connection got unexpected err:%v\n", err)
				}
				go c.stop()
				return
			}
			...
			if c.split != nil {
				...
				if err != nil {
					log.Warn("tcp split packet err:%v\n", err)
					go c.stop()
					return
				}
				...
				for _, pkt := range pkts {
					...
					case c.recvQ <- pkt:
					default:
						log.Warn("recvQ of %v is full, drop packet\n", c.RemoteAddr())
					}
				}
			}
		}
...
func (c *tcpConn) send() {
	...
	for {
		...
		case pkt := <-c.sendQ:
			...
			if err != nil {
				log.Warn("send to %v failed:%v , close connection\n",
					c.RemoteAddr(), err)
				go c.stop()
			}
		}

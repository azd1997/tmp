...
// Id 对于已生成的交易，获取其ID
func (tx *TxP2H) ID() ecrypto.Hash {
	return tx.Id
}

// Hash 计算交易哈希值，作为交易ID
func (tx *TxP2H) Hash() (hash ecrypto.Hash, err error) {
	txCopy := *tx
	txCopy.Id, txCopy.Sig = ecrypto.Hash{}, ecrypto.Signature{}
	var res []byte
	if res, err = txCopy.Serialize(); err != nil {
		return ecrypto.Hash{}, utils.WrapError("TxP2H_Hash", err)
	}
...
// IsValid 验证交易是否合乎规则
func (tx *TxP2H) IsValid(txFunc ValidateTxFunc) (err error) {
	...
	if err = txFunc(tx); err != nil {
		return utils.WrapError("TxP2H_IsValid", err)
	}
